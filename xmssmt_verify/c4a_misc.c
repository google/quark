                       ...
                       const xmssmt_params_t *params )
{
    ...
    if ( out && key && params && m0 )
    {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            c4a_sha256_update( &ctx, key, keyLen );
            c4a_sha256_update( &ctx, m0, mLen );
    ...
    if ( pk0 && publicKey && SEED && ADRS && params )
    {
        ...
        while ( len > 1 )
        {
            for ( uint32_t i = 0; i < (len >> 1); i++ )
            {
                ...
                sprintf(prefix, "pk[%2d]: ", 2*i);
                printVector(prefix, publicKey + (2*i)*params->n, params->n, 32);
                sprintf(prefix, "pk[%2d]: ", 2*i + 1);
                printVector(prefix, publicKey + (2*i + 1)*params->n, params->n, 32);
                ...
                sprintf(prefix, "pk[%2d]: ", i);
                printVector(prefix, publicKey + i*params->n, params->n, 32);
            }
            if ( len & 0x1 )
            {
                          ...
                          const xmssmt_params_t *params )
{
    ...
    if ( pk0 && publicKey && SEED && ADRS && params )
    {
        ...
        memcpy( &stack[stackIdx++][0], publicKey + (treeIndex[0]++)*params->n, params->n );
        treeHeight[stackIdx] = 0;
        memcpy( &stack[stackIdx++][0], publicKey + (treeIndex[0]++)*params->n, params->n );
                ...
                ( ( treeIndex[0] >= params->p ) && ( stackIdx > 1 ) ) )
        {
                 ...
                 ( ( treeIndex[0] >= params->p ) && ( stackIdx >  1 ) ) )
            {
                ...
                ADRS[ADRS_TREE_INDEX]  = (uint32_t) treeIndex[treeHeight[stackIdx - 2] + 1]++;
