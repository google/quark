...
typedef struct {
  uint8_t
      H;          /* Enumerated value representing hash function to use          */
  uint8_t
      w;          /* Base used for interpreting OTS coefficients                 */
  uint8_t
      log2w;      /* log2(w) = # of bits per OTS coefficient                     */
  uint8_t
      p1;         /* # of base-w digits in hash function output                  */
  uint8_t
      p2;         /* # of base-w digits in checksum function output              */
  uint8_t
      p2Bytes;    /* # of bytes in checksum                                      */
  uint8_t
      ls;         /* # of bits to left-shift checksum result by to byte-align it */
  uint8_t
      p;          /* # of base-w digits in ???                                   */
  uint8_t
      h;          /* # of levels in XMSS tree (constant for all sub trees)       */
  uint8_t
      hTotal;     /* total # of levels in XMSS-MT configuration                  */
  uint8_t
      d;          /* # of trees in XMSS-MT                                       */
  uint8_t
      n;          /* # of bytes in hash function output                          */
  uint8_t
      idxLen;     /* length (in bytes) of idx values                             */
  size_t
      otsSigLen;  /* length (in bytes) of WOTS+ signature                        */
  size_t
      xmssSigLen; /* length (in bytes) of XMSS signature                         */
  size_t
      pubKeyLen;  /* length (in bytes) of public key                             */
} xmssmt_params_t;
