 ...
 *
 * @param[in]  flash_id  An enum that specifies which flash ID to read from (ignore from now).
...
sig_retcode_t computeH(uint8_t* out,
  ...
  if (out && key && m && params) {
    ...
    quark_sha256_update(&ctx, key, params->n);
    quark_sha256_update(&ctx, m, 2 * params->n);
...
sig_retcode_t computeHmsgFlash(uint8_t* out,
  ...
  if (out && r && root && m && params) {
    ...
    /* Allocate memory */
    if ((sizeof(QUARK_SHA256_CTX) + MAX_HASH_LEN) > scratchLen) {
      ...
      /* Complete the computation */
      quark_sha256_update(&ctx, r, params->n);
      quark_sha256_update(&ctx, root, params->n);
      writeBigEndian(idxBuff, idx, params->n);
      quark_sha256_update(&ctx, idxBuff, params->n);
      quark_sha256_update(&ctx, m, mLen);
      quark_sha256_final(&ctx, out);
...
sig_retcode_t randHashFlash(uint8_t* out,
  ...
  if (out && left && right && SEED && ADRS && params) {
    if ((3 * MAX_HASH_LEN + sizeof(uint32_t) * ADRS_WORD_COUNT) > scratchLen) {
      ...
      writeBigEndian(adrsBytes + sizeof(uint32_t) * ADRS_KEY_MASK,
                     ADRS[ADRS_KEY_MASK],
                     4);
...
sig_retcode_t computeF(uint8_t* out,
  ...
  if (out && key && params && m0 && m1) {
    ...
    quark_sha256_update(&ctx, m0, params->n);
...
sig_retcode_t computeChainFlash(uint8_t* Y,
  ...
  if (Y && SEED && ADRS && params) {
    if ((MAX_HASH_LEN + MAX_HASH_LEN + sizeof(uint32_t) * ADRS_WORD_COUNT)
        > scratchLen) {
      retVal = SIG_INSUFFICIENT_MEMORY;
    } else {
      key = scratchBuff;
      ...
      scratchBuff +=
          MAX_HASH_LEN + MAX_HASH_LEN + (sizeof(uint32_t) * ADRS_WORD_COUNT);
      scratchLen -=
          MAX_HASH_LEN + MAX_HASH_LEN + (sizeof(uint32_t) * ADRS_WORD_COUNT);
...
sig_retcode_t computeLeafFlash(uint8_t* computedLeaf,
  ...
  if (computedLeaf && message && SEED && adrsOts && adrsLtree && params) {
    if (((STACK_DEPTH * MAX_HASH_LEN) + (STACK_DEPTH + 1) + STACK_DEPTH)
        > scratchLen) {
      retVal = SIG_INSUFFICIENT_MEMORY;
    } else {
      ...
      treeHeight = (uint8_t * )(
          scratchBuff + (STACK_DEPTH * MAX_HASH_LEN) + (STACK_DEPTH + 1));
      scratchBuff +=
          (STACK_DEPTH * MAX_HASH_LEN) + (STACK_DEPTH + 1) + STACK_DEPTH;
      scratchLen -=
          (STACK_DEPTH * MAX_HASH_LEN) + (STACK_DEPTH + 1) + STACK_DEPTH;
    ...
    if (SIG_OK == retVal) {
      ...
      while ((treeIndex[0] < params->p) ||
        ...
        if (((treeIndex[0] < params->p) && (stackIdx >= 2)
          ...
          retVal = computeOtsPkIFlash(&stack[MAX_HASH_LEN * stackIdx++ + 0],
                                      sigOtsOffset,
                                      otsCoeff(chksumBytes,
                                               treeIndex[0] - params->p1,
                                               params->log2w),
                                      SEED,
                                      adrsOts,
                                      treeIndex[0],
...
sig_retcode_t computeLeaf(uint8_t* computedLeaf,
  ...
  /* Validate parameters */
  if (computedLeaf && message && sigOts && SEED && adrsOts && adrsLtree
      && params) {
    /* Stack storage */
    uint8_t stack[STACK_DEPTH][MAX_HASH_LEN];
...
sig_retcode_t xmssVerifySignature(const void* message,
  ...
  if (message && signature && publicKey) {
    ...
    if (SIG_OK == retVal) {
      /* Perform signature validation which is the same as for XMSS-MT with d = 1, */
      /* which was set during parseXmssOid( )                                      */
      retVal = xmssMtVerifySignatureCore(message,
                                         messageLen,
                                         signature,
                                         signatureLen,
                                         publicKey,
  ...
  if ((3 * adrsSize + MAX_HASH_LEN + MAX_IDX_LEN) > scratchLen) {
    ...
    if (SIG_OK == retVal) {
      ...
      for (int i = 0; i < params->d; i++) {
        ...
        adrsOts[ADRS_TREE1] = adrsLtree[ADRS_TREE1] = adrsNode[ADRS_TREE1] =
            (idx >> 32) & 0xFFFFFFFF;
        adrsOts[ADRS_TREE0] = adrsLtree[ADRS_TREE0] = adrsNode[ADRS_TREE0] =
            idx & 0xFFFFFFFF;
    ...
    if (SIG_OK == retVal) {
      ...
      retVal = (0 == memcmp(publicKey + XMSS_PUB_ROOT_OFFSET, root, params->n))
               ? SIG_OK : SIG_INVALID_SIG;
    }
  }
