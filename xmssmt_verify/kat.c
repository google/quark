...
//        printf("g_flashCnt = %u (%u bytes)\n", g_flashCnt, g_flashBytesRead);
        ...
        msg[8] ^= 0x1;
        if ( SIG_OK == result )
            printf( "[RESULTS] corrupted xmssVerifySignatureFlash computation: FAILED\n" );
        else
            printf( "[RESULTS] corrupted xmssVerifySignatureFlash computation: PASSED\n" );
//        printf("g_flashCnt = %u (%u bytes)\n", g_flashCnt, g_flashBytesRead);
    }
...
//        printf("g_flashCnt = %u (%u bytes)\n", g_flashCnt, g_flashBytesRead);
        ...
        msg_mt[8] ^= 0x1;
        if ( SIG_OK == result )
            printf( "[RESULTS] corrupted xmssMtVerifySignatureFlash computation: FAILED\n" );
        else
            printf( "[RESULTS] corrupted xmssMtVerifySignatureFlash computation: PASSED\n" );
//        printf("g_flashCnt = %u (%u bytes)\n", g_flashCnt, g_flashBytesRead);
    }
