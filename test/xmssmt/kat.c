...
int main(void) {
  ...
  uint8_t msg_corrupt[sizeof(msg)];
  memcpy(msg_corrupt, msg, sizeof(msg_corrupt));
  msg_corrupt[8] ^= 0x1;

  uint8_t msg_mt_corrupt[sizeof(msg_mt)];
  memcpy(msg_mt_corrupt, msg, sizeof(msg_mt_corrupt));
  msg_mt_corrupt[8] ^= 0x1;
  ...
  if (1) {
    ...
    /* Tweak the message by a single bit and do it again, taking care to */
    /* pass iff signature result is false since we've corrupted it       */
    result = xmssMtVerifySignature(msg_mt_corrupt,
                                   msgLen_mt,
                                   sig_mt,
                                   sigLen_mt,
                                   pk_mt,
  ...
  if (1) {
    ...
    g_flashCnt = 0;
    g_flashBytesRead = 0;
    result = xmssMtVerifySignatureFlash(msg_mt_corrupt,
                                        msgLen_mt,
                                        signatureOffset,
                                        sigLen_mt,
                                        pk_mt,
