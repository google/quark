 ...
 * @return void
 */
void printLmsSignature( char *prefix, uint8_t *sig, uint32_t *n, uint32_t *p, uint32_t *m, uint32_t *h )
{
    param_set_t otsType, lmsType;
    char str[MAX_PREFIX_LEN];
    if ( prefix && sig && p && n && m && h && (MAX_PREFIX_LEN < strlen( prefix )) )
    {
        otsType = readBigEndian( sig + LMS_SIG_OTS_SIG_OFFSET + OTS_SIG_TYPE_OFFSET, TYPE_LEN );
        otsParameterLookup( otsType, NULL, n, NULL, p, NULL );
        lmsType =  readBigEndian( sig + LMS_SIG_TYPE_OFFSET( *n, *p ), TYPE_LEN );
        lmsParameterLookup( lmsType, NULL, m, h );
        printf(            "%s         q = %d\n", prefix, (int) readBigEndian( sig + LMS_SIG_q_OFFSET, q_LEN ) );
                                  ...
                                  size_t         publicKeyLen )
{
    ...
    uint32_t       n;                // Length (in bytes) of hash function output and K/Kc
    ...
    if ( message && signature && publicKey )
    {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            retVal = otsComputeKc( Kc, I, q, message, messageLen, signature, signatureLen, pubType );
        }
        ...
        if ( SIG_OK == retVal )
        {
            ...
            retVal = ( 0 == memcmp( Kc, publicKey + OTS_KEY_K_OFFSET, n ) ) ? SIG_OK : SIG_INVALID_SIG;
        }
    }
 ...
 *         error code othewise
 */
sig_retcode_t lmsValidatePublicKey( const uint8_t *publicKey, size_t publicKeyLen )
{
    sig_retcode_t retVal = SIG_INVALID_PARAM;
    param_set_t   lmsType;
    size_t        lmsKeyLen;
 ...
 *         error code othewise
 */
sig_retcode_t otsValidatePublicKey( const uint8_t *publicKey, size_t publicKeyLen )
{
    sig_retcode_t retVal = SIG_INVALID_PARAM;
    param_set_t   otsType;
    size_t        otsKeyLen;
