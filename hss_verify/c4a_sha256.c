...
#define C4A_SHA256_FINALCOUNT_SIZE      8
#define C4A_SHA256_K_SIZE               64

/* If C4A_USE_SHA256_C is defined then the C-version of SHA256 compression     */
/* funtion will be used, otherwise the ASM-version of the compression function */
/* will be used.                                                               */
#define C4A_USE_SHA256_C

#ifdef C4A_USE_SHA256_C
static const uint32_t K[C4A_SHA256_K_SIZE] = {
 ...
 * @return void
 */
static void c4a_sha256_c_compress (C4A_SHA256_CTX * ctx, const void *buf)
{
    uint32_t S0, S1, S2, S3, S4, S5, S6, S7, W[C4A_SHA256_K_SIZE], t0, t1, t;
    int i;
    const uint8_t *p;
    ...
    /* fill W[16..63] */
    for (i = 16; i < C4A_SHA256_K_SIZE; i++) {
...
#else /* C4A_USE_SHA256_C is NOT defined */
extern void c4a_sha256_asm_compress(uint32_t *ctxH, uint8_t *ctxData);
#endif /* C4A_USE_SHA256_C */
 ...
 * @return void
 */
void c4a_sha256_init (C4A_SHA256_CTX *ctx)
{
    ctx->Nl = 0;
    ctx->Nh = 0;
    ctx->num = 0;
 ...
 * @return void
 */
void c4a_sha256_update (C4A_SHA256_CTX *ctx, const void *src, uint32_t count)
{
...
#ifdef C4A_USE_SHA256_C
        c4a_sha256_c_compress( ctx, ctx->data );
#else
        c4a_sha256_asm_compress( ctx->h, ctx->data );
#endif
    }
}
 ...
 */
void c4a_sha256_final (C4A_SHA256_CTX *ctx, uint8_t *digest)
{
    uint32_t i;
    uint8_t  finalcount[C4A_SHA256_FINALCOUNT_SIZE];

    writeBigEndian( &finalcount[0], ctx->Nh, 4 );
    writeBigEndian( &finalcount[4], ctx->Nl, 4 );

    c4a_sha256_update(ctx, "\200", 1);

    if (ctx->num > 56) {
        c4a_sha256_update(ctx, "\0\0\0\0\0\0\0\0", 8);
    }
    memset( ctx->data + ctx->num, 0, 56 - ctx->num );
    ctx->num = 56;
    c4a_sha256_update(ctx, finalcount, C4A_SHA256_FINALCOUNT_SIZE);  /* Should cause a sha256_compress() */
