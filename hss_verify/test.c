...
int main( int argc, char **argv )
{
    ...
    passed = 1;
    if ( SIG_INVALID_PARAM != (retVal = otsComputeKc( NULL, NULL, 0, NULL, 0, NULL, 0, -1 ) ) )
    {
        printf( "    FAIL (otsComputeKc): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_PARAM != (retVal = lmsVerifySignature( NULL, 0, NULL, 0, NULL, 0 ) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_PARAM != (retVal = hssVerifySignature( NULL, 0, NULL, 0, NULL, 0 ) ) )
    {
    ...
    passed = 1;
    if ( SIG_INVALID_LMS_KEY_LEN != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 12 + n*(p + 1) + m*h, lmsKey, 0) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_LMS_KEY_LEN != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 12 + n*(p + 1) + m*h, lmsKey, 23 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_HSS_KEY_LEN != (retVal = hssVerifySignature(msg, msgLen, hssSig, sigLen, hssKey, 0) ) )
    {
        printf( "    FAIL (hssVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    ...
    passed = 1;
    hssSig[0] ^= 0x1;
    if ( SIG_INVALID_HSS_LEVELS != (retVal = hssVerifySignature(msg, msgLen, hssSig, sigLen, hssKey, keyLen) ) )
    {
        printf("    FAIL (hssVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    ...
    passed = 1;
    if ( SIG_INVALID_LMS_SIG_LEN != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 0, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_LMS_SIG_LEN != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 11 + n*(p + 1) + m*h, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_OTS_SIG_LEN != (retVal = otsComputeKc(buff, buff, 0, msg, msgLen, otsSig, 3 + n*(p+1), otsType) ) )
    {
        printf( "    FAIL (otsComputeKc): %s\n", retcode_strings[retVal]);
        passed = 0;
    }
    ...
    passed = 1;
    if ( SIG_INVALID_OTS_TYPE != (retVal = otsParameterLookup( -1, NULL, NULL, NULL, NULL, NULL ) ) )
    {
        printf( "    FAIL (otsParameterLookup): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_OTS_TYPE != (retVal = otsComputeKc(buff, buff, 0, buff, 100, buff, 100, -1) ) )
    {
        printf( "    FAIL (otsComputeKc): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    if ( SIG_INVALID_OTS_TYPE != (retVal = otsComputeKc(buff, buff, 0, buff, 100, buff, 100, 0) ) )
    {
        printf( "    FAIL (otsComputeKc): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    lmsSig[4] ^= 0x1;
    if ( SIG_INVALID_OTS_TYPE != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, sigLen, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    lmsSig[4] ^= 0x1;
    if ( SIG_INVALID_LMS_TYPE != (retVal = lmsParameterLookup( -1, NULL, NULL, NULL ) ) )
    {
        printf( "    FAIL (lmsParameterLookup): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    lmsKey[0] ^= 0x01;
    if ( SIG_INVALID_LMS_TYPE != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, sigLen-4, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    lmsKey[0] ^= 0x01;
    lmsSig[8 + n*(p+1)] ^= 0x1;
    if ( SIG_INVALID_LMS_TYPE != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 12 + n*(p + 1) + m*h, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    ...
    passed = 1;
    lmsSig[0] ^= 0x1;
    if ( SIG_INVALID_LMS_NODE != (retVal = lmsVerifySignature(msg, msgLen, lmsSig, 12 + n*(p + 1) + m*h, lmsKey, 24 + m) ) )
    {
        printf( "    FAIL (lmsVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    ...
    passed = 1;
    hssKey[12] ^= 0x1;
    if ( SIG_INVALID_SIG != (retVal = hssVerifySignature(msg, msgLen, hssSig, sigLen, hssKey, keyLen) ) )
    {
        printf( "    FAIL (hssVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
    ...
    printf("SIG_OK tests:\n");
    passed = 1;
    if ( SIG_OK != (retVal = hssVerifySignature(msg, msgLen, hssSig, sigLen, hssKey, keyLen) ) )
    {
        printf( "    FAIL (hssVerifySignature): %s\n", retcode_strings[retVal] );
        passed = 0;
    }
