                                       ...
                                       size_t         scratchLen )
{
    ...
    if ( message && publicKey && scratchBuff )
    {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            {
#ifdef C4A_USE_FLASH_CACHE
                retVal = flashcpy_cached( scratchBuff, signatureOffset + HSS_SIG_LEVELS_OFFSET, L_LEN );
#else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            for ( uint32_t i = 0; i <= sigLevels; i++ )
            {
                ...
                else
                {
                    ...
                    {
#ifdef C4A_USE_FLASH_CACHE
                        retVal = flashcpy_cached( scratchBuff, iterSigOffset + lmsSigLen, lmsKeyLen );
#else
...
{
    sig_retcode_t  retVal = SIG_OK;
    uint8_t       *tmp;                  /* Scratch buffer used during T[i] computations       */
    const uint8_t *I;                    /* Pointer to I value in public key                   */
    const uint8_t *T1;                   /* Pointer to T[1] value in public key                */
    const uint8_t *sigBuff;              /* Pointer to {typeOts} storage in scratchBuff        */
    param_set_t    pubLmsType;           /* LMS type embedded in LMS public key                */
    param_set_t    pubOtsType;           /* OTS type embedded in LMS public key                */
    param_set_t    sigLmsType;           /* LMS type embedded in LMS signature                 */
    param_set_t    sigOtsType;           /* OTS type embedded in LMS signature                 */
    union hash_ctx ctx;                  /* Hash context used for computing T[i] values        */
    uint32_t       H_lms;                /* LMS-related hash function                          */
    uint32_t       m;                    /* LMS-related node data width                        */
    uint32_t       h;                    /* LMS-related tree height                            */
    uint32_t       q;                    /* OTS-related node value                             */
    uint32_t       n;                    /* OTS-related hash function output width             */
    uint32_t       p;                    /* OTS-related element length                         */
    merkle_index_t nodeNum;              /* Merkle tree node number that tracks our tree climb */
    ...
    if ( message && publicKey && scratchBuff )
    {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            else
            {
                ...
                else if ( SIG_OK != otsParameterLookup( pubOtsType, NULL, &n, NULL, &p, NULL ) )
                {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            {
#ifdef C4A_USE_FLASH_CACHE
                retVal = flashcpy_cached( scratchBuff, signatureOffset, q_LEN + TYPE_LEN );
#else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            /* memory space check                                                      */
#ifdef C4A_USE_FLASH_CACHE
            if ( SIG_OK != flashcpy_cached( (uint8_t *)sigBuff, signatureOffset + LMS_SIG_TYPE_OFFSET( n, p), TYPE_LEN ) )
#else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            if ( nodeNum & 0x1 )
            {
                c4a_hash( H_lms, &ctx, tmp + TREEINT_PK_OFFSET + n, tmp, TREELEAF_OFFSET( n ) );
#ifdef C4A_USE_FLASH_CACHE
            ...
            {
                c4a_hash( H_lms, &ctx, tmp + TREEINT_PK_OFFSET, tmp, TREELEAF_OFFSET( n ) );
#ifdef C4A_USE_FLASH_CACHE
                retVal = flashcpy_cached( tmp + TREELEAF_PK_OFFSET + n, signatureOffset, n );
#else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            while ( nodeNum > 1 )
            {
                ...
                if ( nodeNum & 0x1 )
                {
                    c4a_hash( H_lms, &ctx, tmp + TREELEAF_PK_OFFSET + n, tmp, TREEINT_OFFSET( n ) );
#ifdef C4A_USE_FLASH_CACHE
                    retVal = flashcpy_cached( tmp + TREELEAF_PK_OFFSET, signatureOffset, n);
#else
                    retVal = flashcpy( tmp + TREELEAF_PK_OFFSET, signatureOffset, n);
                ...
                {
                    c4a_hash( H_lms, &ctx, tmp + TREELEAF_PK_OFFSET, tmp, TREEINT_OFFSET( n ) );
#ifdef C4A_USE_FLASH_CACHE
                    retVal = flashcpy_cached( tmp + TREELEAF_PK_OFFSET + n, signatureOffset, n);
#else
                    retVal = flashcpy( tmp + TREELEAF_PK_OFFSET + n, signatureOffset, n);
        ...
        if ( SIG_OK == retVal )
        {
            ...
            if ( nodeNum & 0x1 )
            {
                retVal = ( 0 == memcmp( T1, tmp + TREELEAF_PK_OFFSET + n, n ) ) ? SIG_OK : SIG_INVALID_SIG;
            }
            else
            {
                retVal = ( 0 == memcmp( T1, tmp + TREELEAF_PK_OFFSET, n ) ) ? SIG_OK : SIG_INVALID_SIG;
            }
        }
    }
    else
...
{
    sig_retcode_t  retVal = SIG_OK;
    uint8_t        tmp[TREEINT_MAX_LEN]; /* Scratch buffer used during T[i] computations       */
    const uint8_t *I;                    /* Pointer to I value in public key                   */
    const uint8_t *T1;                   /* Pointer to T[1] value in public key                */
    param_set_t    pubLmsType;           /* LMS type embedded in LMS public key                */
    param_set_t    pubOtsType;           /* OTS type embedded in LMS public key                */
    ...
    if ( message && signature && publicKey )
    {
        ...
        if ( SIG_OK == retVal )
        {
            ...
            sigOtsType = readBigEndian( signature + LMS_SIG_OTS_SIG_OFFSET + OTS_SIG_TYPE_OFFSET, TYPE_LEN );
            if ( ( sigOtsType != pubOtsType ) ||
                 ( SIG_OK != otsParameterLookup( pubOtsType, NULL, &n, NULL, &p, NULL ) ) )
            {
                retVal = SIG_INVALID_OTS_TYPE;
            }
            else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            sigLmsType = readBigEndian( signature + LMS_SIG_TYPE_OFFSET( n, p ), TYPE_LEN );
            if ( sigLmsType != pubLmsType )
            {
                retVal = SIG_INVALID_LMS_TYPE;
            }
...
{
    sig_retcode_t  retVal = SIG_OK;
    uint32_t       H;         /* Hash function identifier                                        */
    uint32_t       n;         /* Length of hash function output in bytes                         */
    uint32_t       w;         /* Winternitz value that dictates                                  */
    uint32_t       p;         /* Number of w-bit digits that need to be processed                */
    uint32_t       ls;        /* Left shift index for aligning checksum                          */
    uint32_t       maxDigit;  /* Value 2^w-1 used during Kc computation                          */
    uint8_t       *Q;         /* Array to hold hash output with 2-byte checksum                  */
    uint8_t       *tmp;       /* Array to hold iteration values                                  */
    union hash_ctx ctx;       /* Hashing context for computing Z[i] and Kc values                */
    ...
    if ( computedKc && I && message && scratchBuff )
    {
...
#ifdef C4A_USE_FLASH_CACHE
            retVal = flashcpy_cached( tmp + MSG_C_OFFSET, signatureOffset + TYPE_LEN, n );
#else
        ...
        if ( SIG_OK == retVal )
        {
            ...
            c4a_hashInit( H, &ctx );
            c4a_hashUpdate( H, &ctx, tmp, MSG_MSG_OFFSET(n) );
            c4a_hashUpdate( H, &ctx, message, messageLen );
            c4a_hashFinal( H, &ctx, Q );
            ...
            c4a_hashUpdate( H, &ctx, tmp, OTS_Z_OFFSET );
            ...
            for ( uint32_t i = 0; i < p; i++ )
            {
                ...
                for ( uint32_t j = a; j < maxDigit; j++ )
                {
                    ...
                    tmp[OTS_j_OFFSET] = j;
                    c4a_hash( H, &iterCtx, tmp + OTS_TMP_OFFSET, tmp, OTS_OFFSET(n) );
                }

                /* Mix Z[i] into the computation of Kc */
                c4a_hashUpdate( H, &ctx, tmp + OTS_TMP_OFFSET, n );
            }

            /* Complete the Kc computation */
            c4a_hashFinal( H, &ctx, computedKc );
            retVal = SIG_OK;
        }
    }
    else
...
{
    sig_retcode_t  retVal = SIG_OK;
    param_set_t    sigType;             /* Signature type extracted from signature itself         */
    uint32_t       H;                   /* Hash function identifier                               */
    uint32_t       n;                   /* Length of hash function output in bytes                */
    uint32_t       w;                   /* Winternitz value that dictates                         */
    uint32_t       p;                   /* Number of w-bit digits that need to be processed       */
    uint32_t       ls;                  /* Left shift index for aligning checksum                 */
    uint32_t       maxDigit;            /* Value 2^w-1 used during Kc computation                 */
    const uint8_t *C;                   /* Pointer to n-byte randomizer vector in signature       */
    const uint8_t *Y;                   /* Pointer to beginning of Y[0], ..., Y[p-1] in signature */
    uint8_t        Q[MAX_HASH_LEN + 2]; /* Array to hold hash output with 2-byte checksum         */
    uint8_t        tmp[OTS_MAX_LEN];    /* Array to hold iteration values                         */
    union hash_ctx ctx;                 /* Hashing context for computing Z[i] and Kc values       */
        ...
        if ( SIG_OK == retVal )
        {
            ...
            else
            {
                ...
                c4a_hashInit( H, &ctx );
                {
                    ...
                    c4a_hashUpdate( H, &ctx, buff, MSG_MSG_OFFSET(n) );
                }
                c4a_hashUpdate( H, &ctx, message, messageLen );
                c4a_hashFinal( H, &ctx, Q );
